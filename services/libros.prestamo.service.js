...
const getLibrosPrestados = async (page) =>{
    try {
    const librosPrestamo =  await page.evaluate((LibroEnPrestamo)=>{
        if(document.querySelector('#tabcontent_Title1>#user_tab_loan>.details_tab_copy>.tabcont_vscroll_full>table>tbody') != null){
            ...
            for(let i=0; i<libros.length - 1; i++){
                ...
                prestamos.push(new LibroEnPrestamo(i+1, codigo.innerText, titulo.innerText, fecha.innerText, multa.innerText));   
            }
            return prestamos
        }
